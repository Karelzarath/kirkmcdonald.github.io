...
def processMods(data, gameDir, modDir, modlist):
	for name, version in modlist["active_mods"].items():
		if (name == "base"):
			data["module_info"]["base"] = {"version": version, "localPath": gameDir + "/data/base/"}
			data["module_info"]["core"] = {"version": version, "localPath": gameDir + "/data/core/"}
		else:
			data["module_info"][name] = {"version": version, "mod_name": "%s_%s" % (name, version), "zip_path": "%s/%s_%s.zip" % (modDir, name, version)}

		print ("mod ", name, ": ", data["module_info"][name])


def main():
	parser = argparse.ArgumentParser()
	parser.add_argument("--sheet_prefix", default="images/sprite-sheet")
	parser.add_argument("--datafile", default="../data.json")
	parser.add_argument("--modfile", default="../gamedata.json")
	parser.add_argument("--moddir", default="C:/Users/karel/AppData/Roaming/Factorio/mods")
	parser.add_argument("--gamedir", default="D:/Games/Steam/steamapps/common/Factorio")
	parser.add_argument("--basemod", default="core", help="The main mod for version and display purposes")
	...
	with open(args.modfile, "r") as f:
		processMods(data, args.gamedir, args.moddir, json.load(f))
		
	item_types = ["ammo", "armor", "blueprint", "blueprint-book", "capsule", "deconstruction-item", "fluid", "gun", "item", "item-with-entity-data", "mining-tool", "module", "rail-planner", "repair-tool", "selection-tool", "spidertron-remote", "tool"]
	...
	items = {}
	fuel = []
	item_attrs = {"category", "effect", "fuel_category", "fuel_value", "icon", "icons", "limitation", "name", "order", "subgroup", "type"}
	for item_type in item_types:
		for name, item in data[item_type].items():
			...
			if subgroup in ("fill-barrel", "bob-gas-bottle", "kr-void", "matter-deconversion", "crash-site"):
				continue
			item["group"] = item_subgroups[subgroup]["group"]
			if "icons" in item:
				item["icon"] = item["icons"][0]["icon"]
				del item["icons"]
			if "icon" not in item:
	...
	for name, recipe in data["recipe"].items():
		if ("allow_as_intermediate" in recipe and ~recipe["allow_as_intermediate"]):
			continue
		missing_attrs = []
		if "icons" in recipe:
			recipe["icon"] = recipe["icons"][0]["icon"]
		for attr in inherited_attrs:
			if attr not in recipe:
				if name not in items:
					if ("result" in recipe and recipe["result"] in items):
						recipe[attr] = items[recipe["result"]][attr]
						continue
					missing_attrs.append(attr)
					continue
				if (attr not in items[name]):
					print ("recipe '", name, "' and item missing attribute ", attr, ":", item)
				recipe[attr] = items[name][attr]
		if any(attr not in recipe for attr in inherited_attrs):
			print ("rejecting recipe", recipe["name"], "for missing attributes ", missing_attrs)
			#print (recipe)
			#print ()
			continue
		if recipe["subgroup"] in {"empty-barrel", "fill-barrel", "kr-void", "matter-deconversion", "crash-site"}:
			continue
		...
		if "expensive" in recipe:
			...
			#no expensive recipe, but only a normal one
			#CURSE YOU SPIDERTRON!
			normal = recipe.copy()
			del normal["normal"]
			normal.update(recipe["normal"])
			normalize_recipe(normal)
			normal_recipes[name] = normal
			expensive_recipes[name] = normal
		else:
	...
	## TODO: Turn icon_paths entries into single/multiple
	## TODO: Composite multiple images together to form the final icon
	icon_map = {}
	...
	version = data["module_info"][args.basemod]["version"]
	recipe_pairs = [
		("", normal_recipes),
